...
#ifdef ARGMIN_BENCH_TRACE_ALLOC
// Pre-fix allocation witness for nw_sqp (see micro_kraft_slsqp.cpp for the
// shared rationale). The line-search SQP path allocates per step in the
// current code, so the witness requires the un-blinded gate to observe it
// across the armed step window and reset().
