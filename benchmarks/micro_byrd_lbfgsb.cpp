...
constexpr std::size_t byrd_lbfgsb_bound_active_witness = 4;

}

// byrd_lbfgsb shares the L-BFGS-B hot path. Two scenarios run: the wide-bounds
// all-free path (held to the zero-allocation gate immediately) and a
// bound-active path that exercises the GCP -> free-variable subspace ->
// reduced_hessian / multiply branch. The bound-active scenario carries a
// path-entry assertion so the gate can never certify the claim without actually
// running the branch.
int argmin_alloc_trace_probe()
{
    ...
    // must stay so.
    {
        ...
        rc |= argmin::detail::bench::evaluate_gate("byrd_lbfgsb", 2 * hot_steps, 0);
    }
    ...
    // reduced_hessian / multiply).
    {
        ...
        rc |= argmin::detail::bench::evaluate_gate(
            "byrd_lbfgsb_bound_active", 2 * hot_steps, byrd_lbfgsb_bound_active_witness);
    }
