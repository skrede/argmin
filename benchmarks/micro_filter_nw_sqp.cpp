// Micro-benchmark: argmin filter_nw_sqp vs NLopt LD_SLSQP on constrained HS problems.
//
// Filter NW-SQP uses Fletcher-Leyffer 2002 filter acceptance with the
// active-set QP solver and dense BFGS Hessian. Tested on problems with
// inequality and mixed constraints.
//
                    ...
                    std::optional<double> gamma_h = std::nullopt)
{
    ...
    // Warmup.
    {
        argmin::step_budget_solver solver{argmin::filter_nw_sqp_policy<>{},
...
bool probe_kkt_residual()
{
    ...
    if(!any_kkt)
    {
        argmin::bench::println("FAIL: kkt_residual not populated (filter_nw_sqp)");
        return false;
    }
    ...
    argmin::bench::println("  filter_nw_sqp HS043 kkt_residual: {:.6e} (gradient_norm: {:.6e})",
                 last_with_kkt.kkt_residual.value(),
                 last_with_kkt.gradient_norm);
    return true;
}

// Regression probe: filter_nw_sqp on HS026 must reach f < 1e-5 after
...
bool probe_regression_hs026()
{
    ...
    argmin::step_budget_solver solver{
        argmin::filter_nw_sqp_policy<argmin::hs026<>::problem_dimension>{},
        p, x0, opts};
    ...
    if(!ok)
        argmin::bench::println(stderr,
                     "FAIL: filter_nw_sqp HS026 f={:.6e} kkt={:.6e}",
                     last.objective_value, kkt);
    argmin::bench::println("  filter_nw_sqp HS026: f={:.6e} kkt={:.6e}",
                 last.objective_value, kkt);
    return ok;
}
...
bool probe_regression_hs024_iter_bound()
{
    ...
    argmin::step_budget_solver solver{
        argmin::filter_nw_sqp_policy<argmin::hs024<>::problem_dimension>{},
        p, x0, opts};
    ...
    argmin::bench::println("  filter_nw_sqp HS024: iters={} f={:.6e}",
                 result.iterations, result.objective_value);
    return ok;
}
...
#ifdef ARGMIN_BENCH_TRACE_ALLOC
// Characterized-residual allocation witness for filter_nw_sqp. The probe uses
// a fixed-N HS071 fixture and fixed policy instantiation; it remains in witness
...
int main(int argc, char** argv)
{
    ...
    if(cli_gamma_f && cli_gamma_h)
    {
        ...
        auto emit = [gf, gh](std::string_view name, const sweep_row& r) {
            argmin::bench::println("filter_nw_sqp\t{:.0e}\t{:.0e}\t{}\t{:.10e}\t{:.10e}\t{}\t{}",
    ...
    argmin::bench::println("Filter NW-SQP micro-benchmark, {} repetitions each\n", reps);
