...
|---|---|---|---|
| `move_limit_shrink_policy.h` | Contract `[alpha, beta]` toward `x_k` on non-conservative trial; preserves analytic primal `x_j(y)`. | Original to argmin. | Non-viable: stalls short of `f*` on HS076 and burns wall time on contracted-trust-region dual solves. |
