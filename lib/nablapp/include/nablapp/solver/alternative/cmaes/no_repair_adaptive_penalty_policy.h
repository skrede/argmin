#ifndef HPP_GUARD_NABLAPP_SOLVER_ALTERNATIVE_CMAES_NO_REPAIR_ADAPTIVE_PENALTY_POLICY_H
#define HPP_GUARD_NABLAPP_SOLVER_ALTERNATIVE_CMAES_NO_REPAIR_ADAPTIVE_PENALTY_POLICY_H

// CMA-ES boundary-handling variant: evaluate at the unrepaired x_i and
// add an adaptive boundary penalty
//   penalty = sum_i w_i * (x_i - feasible(x_i))^2
// where w_i is a per-coord adaptive weight tracked via an exponential
// moving average of the diagonal of the rank-mu accumulator
// (Hansen 2009 §3 prescribes the half-life c_c).
//
...
//   + Penalty weights adapt to the local Hessian estimate; less
//     biased away from boundary-active optima on smooth-objective
//     bounded problems.
//   - Two problem.value() evaluations per offspring on bounded cells
//     (one at unrepaired x for search, one at repaired x for the
//     caller-facing unpenalized value).
//   - Adaptive-weight bookkeeping adds per-step state.
//
// CMA-ES algorithmic core: step() = one generation: sample lambda
// offspring, evaluate (with adaptive penalty), rank, update
// mean/covariance/sigma. Supports optional IPOP restart on stagnation
// detection.
//
// Requires: objective<P,S> (no gradient needed).
// When bound_constrained<P,S>, applies the adaptive boundary penalty.
//
// References:
...
//     Optimization", §3 (adaptive boundary penalty).
...
struct no_repair_adaptive_penalty_policy
{
    using scalar_type = double;

    template <int M>
    using rebind = no_repair_adaptive_penalty_policy<M, MaxPopulation>;
    ...
    struct state_type
    {
        ...
        // Per-coord adaptive boundary penalty weights. Initialized to 1
        // in init(); updated each step from a moving average of the
        // diagonal of the rank-mu accumulator (Hansen 2009 §3
        // prescribes the half-life c_c).
        Eigen::Vector<double, N> adaptive_weights;
    };
                    ...
                    const solver_options<Convergence>& opts)
    {
        ...
        // Adaptive boundary penalty weights: initialize to 1 per coord.
        // The exponential moving average in step() updates them from
        // the diagonal of the rank-mu accumulator each generation.
        // Reference: Hansen (2009) §3.
        s.adaptive_weights = Eigen::Vector<double, N>::Ones(n);
    ...
    step_result<double> step(state_type<P>& s)
    {
        ...
        // No-repair adaptive penalty: search distribution sees the
        // unrepaired x_i; CMA-ES does NOT overwrite offspring (the
        // unrepaired coords flow into mean recombination, p_sigma
        // cumulation, and rank-mu deltas below). The objective is
        ...
        Eigen::Matrix<double, N, Eigen::Dynamic, 0,
            ...
            repaired_buf(n, lambda);
        for(int i = 0; i < lambda; ++i)
        {
            const auto xi = offspring.col(i);
            Eigen::Vector<double, N> xi_repaired = xi;
            double penalty = 0.0;
            if(s.has_bounds)
            {
                xi_repaired = xi.cwiseMax(s.lower).cwiseMin(s.upper);
                Eigen::Vector<double, N> diff = xi_repaired - xi;
                penalty = (s.adaptive_weights.array() * diff.array().square()).sum();
                unpenalized[i] = s.problem->value(xi_repaired);
                fitnesses[i] = s.problem->value(xi) + penalty;
            }
            else
            {
                const double f_raw = s.problem->value(xi);
                unpenalized[i] = f_raw;
                fitnesses[i] = f_raw;
            }
            repaired_buf.col(i) = xi_repaired;
        }
        ...
        // 11b. Adaptive boundary penalty weight update (Hansen 2009 §3).
        // The weights track an exponential moving average of the
        // diagonal of the rank-mu accumulator (a Hessian-shape proxy)
        // with half-life c_c. The diagonal is recomputed locally here
        // from the same `deltas` columns used by update_covariance, so
        // no signature change to detail::update_covariance is required.
        if(s.has_bounds)
        {
            Eigen::Vector<double, N> rank_mu_diag =
                Eigen::Vector<double, N>::Zero(n);
            for(int i = 0; i < mu; ++i)
            {
                const auto di = deltas.col(i);
                rank_mu_diag.array()
                    += s.params.weights[i] * di.array().square();
            }
            s.adaptive_weights =
                (1.0 - s.params.c_c) * s.adaptive_weights
                + s.params.c_c * rank_mu_diag.cwiseAbs();
        }
        ...
        // best-ranked offspring is the candidate (ranked by penalized
        // `fitnesses`). For G12, s.x is the REPAIRED (in-box) point of
        ...
        if(gen_best_f < s.objective_value)
        {
            ...
            s.x = repaired_buf.col(best_offspring);
        }
